...
class StudentDB:
    ...
    def setup_db(self):
        try:
            self.conn = mysql.connector.connect(host='localhost', database='students', user='****', password='*******')
    ...
    def create_widgets(self):
        #----ROW 1
        #Create the lable and place it in the upper left hand corner using the grid layout
        sid_label = Label(root, text = "ID")
        sid_label.grid(row=0, column=0,padx=5,pady=10,sticky=W)

        #will hold the values entered int the entry_widget
        self.sid_entry_value = StringVar(root, value="")

        #Create the entry widget and assign all values entered into it to the StringVar
    ...
    #Executes the query and fetches result from the query if it is expected
    ...
    #Clear the tree and then get updated data        
    ...
    def add_student(self):
        ...
        if(self.conn.is_connected() and not self.all_entries_filled(False)):
            ...
            #Get the current time and format it to fit what MySQL expects
