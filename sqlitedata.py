...
# connect() will open an SQL database or if it doesn't exist it will create it
...
def print_db():
    #to retrieve data from a table use SELECT followed by the items to retrieve and the 
    #table to retrieve from
    try:
        result = the_cursor.execute("SELECT id, f_name, l_name, age, address, salary, hire_date FROM employees")

        # You receive a list of list that hold the result
        for row in result:
            print("id: ", row[0])
            print("f_name: ", row[1])
            print("l_name: ", row[2])
...
try:
    ...
    print("Table couln't be created; ", str(e))
...
db_conn.execute("INSERT INTO employees(f_name, l_name, age, address, salary, hire_date) VALUES ('Derek', 'Banas', 43, '123 Main st', 50000, date('now'));");
db_conn.commit()
...
try:
    ...
    print("Database couln't be updated")
